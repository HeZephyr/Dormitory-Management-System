			...
			<table class="table table-hover table-striped table-bordered">
				...
				<c:forEach  varStatus="i" var="dormManager" items="${dormManagers }">
					<tr>
						...
						<th>${dormManager.dormmasterId}</th>
						<td>${dormManager.userName }</td>
						<td>${dormManager.sex }</td>
						<td>${dormManager.tel }</td>
						<td>${dormManager.dorm_id==null?"wu":dormManager.dorm_id }</td>
<%--						<th>???</th>--%>
						<td><button class="btn btn-mini btn-info" type="button" onclick="javascript:window.location='dormManager?action=preSave&dormManagerId=${dormManager.dormmasterId }&password=${dormManager.password }'">修改</button>&nbsp;
							<button class="btn btn-mini btn-danger" type="button" onclick="javascript:window.location='dormManager?action=delete&dormManagerId=${dormManager.dormmasterId}'">删除</button></td>
<%--							<button class="btn btn-mini btn-danger" type="button" onclick="dormManagerDelete(${dormManager.dormmasterId})">删除</button></td>--%>
					</tr>
				</c:forEach>
			</table>
		</div>
