...
<div class="data_list">
		...
		<form action="student?action=save&studentId=${student.studentId}" method="post" onsubmit="return checkForm()">
			<div class="data_form" >
<%--				<input type="hidden" id="studentId" name="studentId" value="${student.studentId }"/>--%>
					<table align="center">
						<c:choose>
							...
							<c:otherwise>
								<tr>
									...
									<td><input type="text" id="userName" name="studentIdNew" value="${student.studentId }"/></td>
								</tr>
							</c:otherwise>
						</c:choose>
						...
						<tr>
							<td><font color="red">*</font>户籍：</td>
							<td><input type="text" id="name"  name="native_place" value="${student.native_place }"  style="margin-top:5px;height:30px;" /></td>
						</tr>
						<tr>
							<td><font color="red">*</font>学院：</td>
							<td>
								<select id="dormBuildId" name="academy" style="width: 90px;">
									<c:forEach var="academy" items="${academyList }">
										<option value="${academy.academy }" ${student.academy==academy.academy?'selected':'' }>${academy.academy}</option>
									</c:forEach>
								</select>
							</td>
						</tr>
						<tr>
							<td><font color="red">*</font>专业班级：</td>
							<td><input type="text" id="name"  name="major_and_class" value="${student.major_and_class }"  style="margin-top:5px;height:30px;" /></td>
						</tr>
						<tr>
							...
							<td>
								<select id="dormBuildId" name="dormBuildId" style="width: 90px;">
									<c:forEach var="dormBuild" items="${dormBuildList }">
										<option value="${dormBuild.dorm_id }" ${student.dorm_id==dormBuild.dorm_id?'selected':'' }>${dormBuild.dorm_name }</option>
									</c:forEach>
								</select>
							</td>
						</tr>
						<tr>
							...
							<td><input type="text" id="dormName"  name="room_id" value="${student.room_id }"  style="margin-top:5px;height:30px;" /></td>
						</tr>
